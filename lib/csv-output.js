...
var fieldNamesVersionChanges = ['Component Name', 'Old Version', 'New Version', 'directory', 'old directory', 'Download URL', 'Changed', 'New License Type', 'Old License Type',
  'License Terms', 'Package.json', 'README'];
...
function changedRecordToPlainJSON(newRec, oldRec){
  if (newRec.licenseSources) { //nlf returns for added.
    ...
    return {
      ...
      'old directory': oldRec.directory,
      'repository': newRec.repository,
      'licenseChanged': newRec.summary().join().toLowerCase() == oldRec.summary().join().toLowerCase()?"F":"T",
      'new summary': newRec.summary().join(';'),
      'old summary': oldRec.summary().join(';'),
      'licenseText': license ? license.text : "",
      'packageJson': (packageLicense ? packageLicense : "(NULL)" + "-" + packageJsonUrl ? packageJsonUrl : "(NULL)") || '',
      'readmeText': "" //readme ? readme.text : ""
    }
